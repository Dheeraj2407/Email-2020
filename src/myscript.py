...
GOOGLE_CLIENT_ID = '839580665554-l90dj6jkcfqflmc9i4qt1tks7ucgm20l.apps.googleusercontent.com'
GOOGLE_CLIENT_SECRET = 'PCKtAgFK1eeyUGQgEqY0jIi3'
...
def call_refresh_token(client_id=GOOGLE_CLIENT_ID, client_secret=GOOGLE_CLIENT_SECRET, refresh_token=GOOGLE_REFRESH_TOKEN):
    params = {}
    params['client_id'] = client_id
    params['client_secret'] = client_secret
...
def get_authorization(ver_code,google_client_id=GOOGLE_CLIENT_ID, google_client_secret=GOOGLE_CLIENT_SECRET):
    scope = "https://mail.google.com/"
    print('Navigate to the following URL to auth:', generate_permission_url(google_client_id, scope))
    authorization_code = ver_code
    response = call_authorize_tokens(google_client_id, google_client_secret, authorization_code)
    return response['refresh_token'], response['access_token'], response['expires_in']


def refresh_authorization(refresh_token,google_client_id=GOOGLE_CLIENT_ID, google_client_secret=GOOGLE_CLIENT_SECRET):
    response = call_refresh_token(google_client_id, google_client_secret, refresh_token)
    return response['access_token'], response['expires_in']


def send_mail(fromaddr, toaddr, subject, message,refresh_token=GOOGLE_REFRESH_TOKEN):
    access_token, expires_in = refresh_authorization(refresh_token)
